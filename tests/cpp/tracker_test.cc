...
TEST(Tracker, ErrorHandling) {
  ...
  RPCUtil conn(tracker_port);
  conn.SendPacket("[]");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");

  conn.SendPacket("");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");

  conn.SendPacket("[42, 3, 4, 5]");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");

  conn.SendPacket("[\"spam\", 3, 4, 5]");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");

  conn.SendPacket("\"spam\"");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");

  conn.SendPacket("[4, 3, 4, 5]");
  status = conn.RecvPacket();
  EXPECT_STREQ(status.c_str(), "-1");
}
