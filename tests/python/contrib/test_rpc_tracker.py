...
def test_tracker():
    try:
        ...
        def _put(tclient, value):
            ...
            print("send", value)
            result = base.recvjson(tclient._sock)
            print("recv", result)
            return result
        ...
        server0 = rpc.Server(
            "localhost", port=9099, tracker_addr=("localhost", tserver.port), key="server0"
        )
        print("A1")
        base.sendjson(tclient._sock, [TrackerCode.REQUEST, "server0", "", 10])
        base.sendjson(tclient._sock, [TrackerCode.REQUEST, "server0", "", 20])
        base.sendjson(tclient._sock, [TrackerCode.REQUEST, "server0", "", 30])
        base.sendjson(tclient._sock, [TrackerCode.REQUEST, "server0", "", 40])

        _put(tclient, [TrackerCode.REQUEST, "server0", "", 10])
        print("A2")
        _put(tclient, [TrackerCode.REQUEST, "server0", "", 20])
        print("A3")
        _put(tclient, [TrackerCode.REQUEST, "server0", "", 30])
        print("A4")
        _put(tclient, [TrackerCode.REQUEST, "server0", "", 40])
        print("B")
        time.sleep(5)
        print("C")
    except ImportError:
